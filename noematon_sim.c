...
#define MAX_SIM_STEPS 3000 // Changed from 50000 to transition to pattern writing sooner
...
#define MAX_ACTIONS 8 // Max actions a worker can associate with a name
#define MAX_RITUALS 32
...
#define GOOD_ENOUGH_MSE 1e-5 // Lowered to push for better reconstruction
#define MIN_TARGET_INFLUENCE 0.5 // Increased minimum target influence to prioritize fidelity
...
struct KernelElement {
    ...
    double angle_delta; // curvature per step for arcs
    int steps; // number of steps for arc
};
...
static void worker_init_kernel(Simulation* sim, Machine *mach) {
    ...
    for (int i = 0; i < mach->kernel.count; i++) {
        ...
        mach->kernel.elements[i].angle_delta = (sim_rand_double(sim) - 0.5) * M_PI / 8; // small curvature
        mach->kernel.elements[i].steps = (int)(sim_rand_double(sim) * 10) + 1; // 1 to 11 steps
    }
}
...
#pragma omp atomic
    ...
    elem.angle_delta += angle_delta_mod;
    elem.steps += (int)steps_mod;
    if (elem.steps < 1) elem.steps = 1;
    double intensity = prediction * elem.weight * c;
    // Apply the arc
    int new_cx, new_cy;
    worker_apply_arc(sim, sim->global_canvas, sim->target_canvas, sim->target_influence_weight, cx, cy, elem.angle_delta, elem.steps, elem.radius, intensity, &new_cx, &new_cy);
    ...
    // Total reward for the worker - Increased image_reward coefficient to prioritize fidelity
    double total_reward = (local_mse_before - local_mse_after) * sim->target_influence_weight * 2.0 + // Boosted image_reward
                          taste_score * (1.0 - sim->target_influence_weight) * 0.5 + // Reduced taste influence
...
static void simulation_adapt_parameters(Simulation* sim) {
    ...
    if (current_mse > GOOD_ENOUGH_MSE) {
        sim->target_influence_weight -= is_stagnating ? sim->params.target_influence_adapt_rate * 1.0 : sim->params.target_influence_adapt_rate * 0.25; // Slower reductions
        char log_msg[256];
        ...
        if (avg_prediction_error > sim->params.taste_influence_threshold_chaos * 2.0) {
            ...
            sim->target_influence_weight -= sim->params.target_influence_adapt_rate * 0.5; // Slower reduction when good
            char log_msg[256];
    ...
    // Clamp to [MIN_TARGET_INFLUENCE, 0.9] to allow higher influence
    sim->target_influence_weight = fmax(MIN_TARGET_INFLUENCE, fmin(0.9, sim->target_influence_weight));
    ...
    if (is_stagnating) {
        sim->params.critic_target_weight -= sim->params.target_influence_adapt_rate * 0.25; // Slower reduction
        sim->params.critic_target_weight = fmax(0.3, sim->params.critic_target_weight); // Higher min
        char log_msg[256];
...
#pragma omp parallel for
    for (int w = 0; w < WORKER_COUNT; w++) {
        ...
        // Apply the arc
        int new_cx, new_cy;
        worker_apply_arc(sim, sim->pattern_canvas, NULL, 0.0, cx, cy, elem.angle_delta, elem.steps, elem.radius, intensity, &new_cx, &new_cy);
        mach->current = (prediction < 0) ? mach->current->next_left : mach->current->next_right;
        if (mach->current) {
            mach->current->target_x = new_cx;
            mach->current->target_y = new_cy;
...
static void init_default_params(Simulation* sim) {
    ...
    sim->params.critic_target_weight = 0.8; // Increased initial critic focus on target
    sim->params.critic_novelty_weight = 0.2; // Reduced novelty weight in critics
    sim->params.critic_repetition_penalty_weight = 0.2; // Reduced repetition penalty
    sim->params.critic_pattern_novelty_weight = 1.0;
    sim->params.critic_pattern_repetition_penalty_weight = 1.0;
    sim->params.taste_novelty_weight = 50.0; // Reduced from 100.0 to de-emphasize novelty
    sim->params.taste_predictive_weight = 20.0; // Reduced from 40.0
    sim->params.target_influence_adapt_rate = 0.01; // Slower adaptation rate
...
static Simulation* simulation_init(void) {
    ...
    sim->target_influence_weight = 0.8; // Start with higher target influence
